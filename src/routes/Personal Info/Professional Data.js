...
proData.push({
  ...
  <div>
    <p>
      When I was working in Tsinghua Energy Internet Research Institution, I found I was able
      to displaymy idea such apps or games throughout graphics instead of terminal. So far I
      used whar I learn in there to build up my server.
    </p>
    <p>
...
proData.push({
  ...
  <div>
    <p>
      Huatech web management system is to help the company monitoring there switchs status.
      System is designed by {'front-back seperation'}. backend will analysis the data sent
      from switch and store into database. The web page shows the data from database, plot
      graph for voltages, current and so on, switch location and export detail data for the
      manager.(these are the features finished when I leave)
    </p>
    <p>
      I was in the team of two(later three). I used Go language to build up client application
      to collect data. My supervisor use beego to build the parse-API library for the golang.
      And I modify it in order to compact with application
    </p>
    <p>
      For web page, team decided to using ant-design pro and dva(umi) at start point because
      it is friendly to beginner. I wrote web page by React and Parse-API-Javascript was used
...
proData.push({
  ...
  <div>
    ...
    <p>
      ...
      for user interface. Better data structure can boost the program. Good API would help teammate
      to build UI.
    </p>
    <p>
      The last part of project was to solve the TSP(Traveller-Salesman Problem) which is exclusive
      from guiding feature. In this part, we had to find the shortest path between many diliver and
      pickup points.I designed the combined algorithms to tackle TSP. I combined 2-opt, greedy
