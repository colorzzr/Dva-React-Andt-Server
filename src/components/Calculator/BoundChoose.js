...
class BoundChoose extends PureComponent {
  render() {
    ...
    const okButton = (<Button id="UpperOK" onClick={finishInputIL}> OK </Button>);
    const list = [];
    // formign list by array
    for (let i = 0; i < boundArr.length; i += 1) {
      list.push(
        <li key={i * 2} >
          <Button id={boundArr[i]} onClick={switchInputWindow}> {boundArr[i]} </Button>
          {okButton}
        </li>,
      );
    ...
    return (
      <ul className={style.BoundChooseInout}>
        {list}
      </ul>
    );
  }
